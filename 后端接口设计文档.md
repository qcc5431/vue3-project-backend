...
> 手机号验证码登录（首次登录自动注册），无需提前创建账号。
> 短信支持三种模式：`mock`（默认，验证码打印到控制台）、`tencent`（腾讯云短信）、`smsbao`（短信宝），通过 `.env` 中的 `SMS_MODE` 切换，**业务代码零改动**。
...
**说明**：
- 手机号 **首次登录自动注册**，`isNewUser: true`
- 登录成功返回 JWT Token，有效期 7 天
- Token 使用方式：`Authorization: Bearer <token>`
  ...
  "data": {
    "list": [
        ...
        "replies": [
          {
            ...
            "userId": "4",
            "username": "小红",
            "userAvatar": "https://api.dicebear.com/7.x/avataaars/svg?seed=yyy",
            "content": "同意！",
            "likeCount": 2,
            "isLiked": false,
            "replyTo": "c1",
          ...
          },
          {
            ...
            "userId": "5",
            "username": "小李",
            "userAvatar": "https://api.dicebear.com/7.x/avataaars/svg?seed=zzz",
            "content": "确实很赞",
            "likeCount": 1,
            "isLiked": false,
            "replyTo": "c1-r1",
      ...
      },
      {
        ...
        "userId": "6",
        "username": "小张",
        "userAvatar": "https://api.dicebear.com/7.x/avataaars/svg?seed=aaa",
        "content": "请问是哪里？",
        "likeCount": 0,
        "isLiked": false,
...
| replyTo | string | 回复的评论ID（仅子回复有此字段） |
| replyToUser | string | 回复的用户名（仅子回复有此字段） |
| **replyCount** | number | **🆕 该评论的回复总数** |
| **replies** | Comment[] | **🆕 子回复列表（嵌套结构，与父评论结构相同）** |

**重要说明**：

1. **只返回顶级评论**：`list` 中只包含没有 `replyTo` 的顶级评论
2. **回复嵌套在 `replies` 中**：每条顶级评论的回复放在其 `replies` 数组中
3. **分页只针对顶级评论**：`page` 和 `pageSize` 只对顶级评论生效
4. **子回复一次性返回**：`replies` 包含该评论下的所有子回复（前端负责"展开更多"逻辑）
5. **子回复可以互相回复**：子回复的 `replyTo` 可以指向另一个子回复ID
...
| 模块   | 方法   | 路径                                 | 说明                   | 认证 |
| ------ | ------ | ------------------------------------ | ---------------------- | ---- |
| 认证   | POST   | /api/auth/send-code                  | 发送手机验证码         | ❌   |
| 认证   | POST   | /api/auth/phone-login                | 手机号登录/自动注册    | ❌   |
...
| 社交   | POST   | /api/social/comments/like            | 点赞/取消点赞评论| ✅   |
